...
tor(r) = toR(r)[:]
function toR2(r)
    det(reshape(real(r),3,3)) > 0 ? toR(r) : toR(-r)
end
...
function calibForceNullspace(POSES,forces)
    N = size(POSES,3)

    F2 = [POSES[:,:,k]'*POSES[:,:,i]*kron(forces[i,:]', Eye(3)) - kron(forces[k,:]', Eye(3)) for i = 1:N-1 for k=i+1:N]
    F = reduce(vcat,F2)
