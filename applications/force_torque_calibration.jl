...
plot!(
    getindex.(errortraces, 2);
    subplot = 2,
    scales...,
    title = "\$g\$ relative error",
)
plot!(
    getindex.(errortraces, 3);
    subplot = 3,
    scales...,
    title = "\$g\$ angle [deg]",
)
...
histogram!(
    [Rangle(t[1], t[4]) * 180 / pi for t in traces];
    subplot = 1,
    opts...,
)

histogram!(
    [e[2][end] for e in errortraces];
    subplot = 2,
    title = "g relative",
    opts...,
)
histogram!(
    [norm(t[2] - t[5]) / norm(t[2]) for t in traces];
    subplot = 2,
    opts...,
)

histogram!(
    [e[3][end] for e in errortraces];
    subplot = 3,
    title = "g angle",
    opts...,
)
